...
def get_configured_plan_names() -> Dict[str, str]:
    day_plan = get_controller_feishu_value("PLAN_DAY_NAME", DEFAULT_FEISHU["PLAN_DAY_NAME"]).strip()
    night_plan = get_controller_feishu_value("PLAN_NIGHT_NAME", DEFAULT_FEISHU["PLAN_NIGHT_NAME"]).strip()
    return {
        "day": day_plan or DEFAULT_FEISHU["PLAN_DAY_NAME"],
        "night": night_plan or DEFAULT_FEISHU["PLAN_NIGHT_NAME"],
    }
...
def split_feishu_command_blocks(text: str, bot_name: str = "") -> List[str]:
    plan_names = get_configured_plan_names()
    command_keywords = [
        ...
        "เปิดไอดี", "dwsa", "กะเช้า", "แพลนเช้า", "เปลี่ยนกะเช้า",
        "เปลี่ยนแพลนเช้า", "กะบ่าย", "แพลนบ่าย", "เปลี่ยนกะบ่าย",
        "เปลี่ยนแพลนบ่าย", "dwsb", "กะดึก", "แพลนดึก", "เปลี่ยนกะดึก",
        "เปลี่ยนแพลนดึก", plan_names["day"].lower(), plan_names["night"].lower(),
    ]
...
def detect_plan_intent(text):
    ...
    plan_names = get_configured_plan_names()
    day_plan = plan_names["day"]
    night_plan = plan_names["night"]
    day_plan_key = normalize_message_text(day_plan)
    night_plan_key = normalize_message_text(night_plan)
    ...
    if not has_plan_action:
        return None
    if contains_any(normalized_text, ["เช้า", "บ่าย", "dwsa", "dws a", day_plan_key]):
        return day_plan
    if contains_any(normalized_text, ["ดึก", "dwsb", "dws b", night_plan_key]):
        return night_plan
    return None
...
def feishu_event():
    ...
    for command_text in command_lines:
        ...
        plan_names = get_configured_plan_names()
        if re.search(r"(เปลี่ยน|สลับ).*(เช้า|บ่าย|dwsa)", normalized_command_lower):
            target_plan = plan_names["day"]
        elif re.search(r"(เปลี่ยน|สลับ).*(ดึก|dwsb)", normalized_command_lower):
            target_plan = plan_names["night"]
...
class App(ctk.CTk):
    ...
    def save_config(self):
        if hasattr(self, "out_entry"):
            ...
            for attr, key in [
                ...
                ("plan_day_entry", "PLAN_DAY_NAME"),
                ("plan_night_entry", "PLAN_NIGHT_NAME"),
            ]:
                if hasattr(self, attr):
                    self.config["FEISHU"][key] = entry_value(
                        getattr(self, attr),
                        collapse_internal_spaces=("TOKEN" in key or key == "BOT_PORT"),
                    )
    ...
    def build_controller_dws_tab(self, tab):
        ...
        alias_card = self.make_card(tab, "#3b4252", 18)
        alias_card.grid(row=1, column=0, padx=10, pady=(0, 10), sticky="ew")
        for col in range(7):
            alias_card.grid_columnconfigure(col, weight=1 if col in {1, 4} else 0)
        ctk.CTkLabel(alias_card, text="Plan Detection Names", text_color="#eceff4", font=ctk.CTkFont(size=15, weight="bold")).grid(row=0, column=0, padx=(16, 8), pady=(12, 4), sticky="w")
        ctk.CTkLabel(alias_card, text="เช้า", text_color="#aeb8cc").grid(row=1, column=0, padx=(16, 8), pady=(4, 14), sticky="w")
        self.plan_day_entry = ctk.CTkEntry(alias_card, placeholder_text="DWSA", width=180)
        self.plan_day_entry.grid(row=1, column=1, padx=(0, 8), pady=(4, 14), sticky="ew")
        ctk.CTkButton(alias_card, text="Use", width=64, fg_color="#4c566a", hover_color="#5e6779", command=lambda: self.use_plan_alias("day")).grid(row=1, column=2, padx=(0, 12), pady=(4, 14))
        ctk.CTkLabel(alias_card, text="ดึก", text_color="#aeb8cc").grid(row=1, column=3, padx=(0, 8), pady=(4, 14), sticky="w")
        self.plan_night_entry = ctk.CTkEntry(alias_card, placeholder_text="DWSB", width=180)
        self.plan_night_entry.grid(row=1, column=4, padx=(0, 8), pady=(4, 14), sticky="ew")
        ctk.CTkButton(alias_card, text="Use", width=64, fg_color="#4c566a", hover_color="#5e6779", command=lambda: self.use_plan_alias("night")).grid(row=1, column=5, padx=(0, 12), pady=(4, 14))
        ctk.CTkButton(alias_card, text="Save Names", width=110, fg_color="#5e81ac", hover_color="#4c6e93", command=self.save_plan_aliases).grid(row=1, column=6, padx=(0, 16), pady=(4, 14))
        for ent in (self.plan_day_entry, self.plan_night_entry):
            ent.bind("<KeyRelease>", lambda _e: self.save_config_debounced(), "+")

        table_card = self.make_card(tab, "#2e3440", 18)
        table_card.grid(row=2, column=0, padx=10, pady=(0, 10), sticky="nsew")
        ...
        log_card.grid(row=3, column=0, padx=10, pady=(0, 10), sticky="nsew")
        log_card.grid_columnconfigure(0, weight=1)
        log_card.grid_rowconfigure(1, weight=1)
        top = ctk.CTkFrame(log_card, fg_color="transparent")
    ...
    def get_feishu_config_value(self, key: str, default: str = "") -> str:
        entry_map = {
            ...
            "PLAN_DAY_NAME": "plan_day_entry",
            "PLAN_NIGHT_NAME": "plan_night_entry",
        }
    ...
    def get_plan_alias_value(self, alias_type: str) -> str:
        if alias_type == "day":
            default = DEFAULT_FEISHU["PLAN_DAY_NAME"]
            attr = "plan_day_entry"
        else:
            default = DEFAULT_FEISHU["PLAN_NIGHT_NAME"]
            attr = "plan_night_entry"
        if hasattr(self, attr):
            return entry_value(getattr(self, attr), collapse_internal_spaces=False) or default
        key = "PLAN_DAY_NAME" if alias_type == "day" else "PLAN_NIGHT_NAME"
        return self.config["FEISHU"].get(key, default) if "FEISHU" in self.config else default

    def use_plan_alias(self, alias_type: str):
        if not hasattr(self, "controller_plan_entry"):
            return
        plan_name = self.get_plan_alias_value(alias_type)
        self.controller_plan_entry.delete(0, "end")
        self.controller_plan_entry.insert(0, plan_name)

    def save_plan_aliases(self):
        self.save_config()
        day_plan = self.get_plan_alias_value("day")
        night_plan = self.get_plan_alias_value("night")
        self.add_log(f"[SYSTEM] Plan detection saved: DAY={day_plan}, NIGHT={night_plan}")
