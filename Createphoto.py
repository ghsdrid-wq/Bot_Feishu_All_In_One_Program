...
QUERY_REFRESH_SMALL_BATCH_SIZE = 6
QUERY_REFRESH_LARGE_BATCH_SIZE = 6
QUERY_REFRESH_LARGE_QUERY_COUNT = 24
QUERY_REFRESH_TIMEOUT_SECONDS = 20 * 60
QUERY_REFRESH_LOG_INTERVAL_SECONDS = 5
EXCEL_CLEANUP_GRACE_SECONDS = 20
...
def wait_for_process_exit(pid: Optional[int], timeout: float = 5.0) -> bool:
...
def kill_excel_pid_if_alive(pid: Optional[int], log: Optional[LogFunc] = None) -> bool:
    ...
    try:
        ...
        if result.returncode == 0 and wait_for_process_exit(pid):
            if log:
                log(f"[CLEANUP] Force-closed leftover Excel PID {pid}")
            return True
        if result.returncode == 0:
            if log:
                log(f"[WARN] Excel PID {pid} still exists after taskkill")
            return False
        if log:
            log(f"[WARN] Could not force-close Excel PID {pid} (taskkill exit code {result.returncode})")
    except Exception as e:
        if log:
            log(f"[WARN] taskkill Excel PID {pid} failed: {e}")
    return False


def start_excel_cleanup_watchdog(
    pid: Optional[int],
    log: Optional[LogFunc] = None,
    grace_seconds: int = EXCEL_CLEANUP_GRACE_SECONDS,
):
    done = threading.Event()
    if not pid:
        if log:
            log("[WARN] Excel PID unavailable; forced cleanup fallback is disabled")
        return done, None

    def force_close_if_stuck() -> None:
        if done.wait(max(1, grace_seconds)):
            return
        if log:
            log(f"[CLEANUP] Excel PID {pid} did not close within {grace_seconds}s")
        kill_excel_pid_if_alive(pid, log)

    watcher = threading.Thread(
        target=force_close_if_stuck,
        name=f"ExcelCleanup-{pid}",
        daemon=True,
    )
    watcher.start()
    return done, watcher
    ...
    try:
        ...
        for sheet in target_sheets:
            try:
                ...
                if log:
                    log(f"[SKIP] Could not trim sheet {sheet}: {e}")
...
def get_workbook_query_tables(wb, write: Optional[LogFunc] = None) -> List[QueryTableRef]:
    """Return stable table locators without retaining refresh-sensitive COM proxies."""
    queries: List[QueryTableRef] = []
    sheet_count = int(excel_call(lambda: wb.Worksheets.Count, write, "count worksheets", timeout=60))
    for sheet_index in range(1, sheet_count + 1):
        ws = excel_call(
            lambda i=sheet_index: wb.Worksheets.Item(i),
            write,
            f"open worksheet {sheet_index}",
            timeout=60,
        )
        sheet_name = str(excel_call(lambda target=ws: target.Name, write, "read worksheet name", timeout=60))
        list_count = int(excel_call(lambda target=ws: target.ListObjects.Count, write, f"count tables {sheet_name}", timeout=60))
        for list_index in range(1, list_count + 1):
            table = excel_call(
                lambda target=ws, i=list_index: target.ListObjects.Item(i),
                write,
                f"open table {sheet_name}:{list_index}",
                timeout=60,
            )
            try:
                excel_call(lambda target=table: target.QueryTable, write, f"read query table {sheet_name}:{list_index}", timeout=60)
            except Exception:
                continue
            try:
                table_name = str(table.Name)
            except Exception:
                table_name = str(list_index)
            queries.append(
                QueryTableRef(
                    label=f"{sheet_name}/{table_name}",
                    sheet_index=sheet_index,
                    table_index=list_index,
                )
            )
    return queries


def resolve_query_table(wb, query_ref: QueryTableRef):
    worksheet = wb.Worksheets.Item(query_ref.sheet_index)
    table = worksheet.ListObjects.Item(query_ref.table_index)
    return table.QueryTable


def cancel_query_refreshes(wb, queries: List[QueryTableRef]) -> None:
    for query_ref in queries:
        try:
            query = resolve_query_table(wb, query_ref)
            if query.Refreshing:
                query.CancelRefresh()
        except Exception:
            pass
...
def refresh_workbook_queries(
    wb,
    excel,
    is_running: Optional[RunFunc],
    write: LogFunc,
    batch_size: Optional[int] = None,
    timeout: int = QUERY_REFRESH_TIMEOUT_SECONDS,
) -> None:
    """Refresh Power Query tables with bounded, workbook-sized concurrency."""
    queries = get_workbook_query_tables(wb, write)
    connection_count = int(excel_call(lambda: wb.Connections.Count, write, "count workbook connections", timeout=60))

    if not queries:
        if connection_count:
            write(f"[REFRESH] Fallback RefreshAll for {connection_count} connection(s)")
            excel_call(lambda: wb.RefreshAll(), write, "refresh workbook connections", timeout=240)
            excel_call(lambda: excel.CalculateUntilAsyncQueriesDone(), write, "wait workbook connections", timeout=timeout)
        else:
            write("[REFRESH] No data connections")
        return

    if connection_count != len(queries):
        write(f"[WARN] Found {connection_count} connection(s), {len(queries)} loaded query table(s)")

    if batch_size is None:
        batch_size = (
            QUERY_REFRESH_LARGE_BATCH_SIZE
            if len(queries) >= QUERY_REFRESH_LARGE_QUERY_COUNT
            else QUERY_REFRESH_SMALL_BATCH_SIZE
        )
    batch_size = max(1, min(int(batch_size), len(queries)))
    total_batches = (len(queries) + batch_size - 1) // batch_size
    refresh_started = time.time()
    write(f"[REFRESH] Starting {len(queries)} queries in {total_batches} batch(es), {batch_size} at a time")

    try:
        for batch_number, start_index in enumerate(range(0, len(queries), batch_size), start=1):
            if is_running and not is_running():
                cancel_query_refreshes(wb, queries)
                return

            batch = queries[start_index:start_index + batch_size]
            write(f"[REFRESH] Batch {batch_number}/{total_batches} started")
            for query_ref in batch:
                excel_call(
                    lambda ref=query_ref: resolve_query_table(wb, ref).Refresh(True),
                    write,
                    f"start query {query_ref.label}",
                    timeout=60,
            ...
            while True:
                ...
                for query_ref in batch:
                    try:
                        refreshing = excel_call(
                            lambda ref=query_ref: bool(resolve_query_table(wb, ref).Refreshing),
                            write,
                            f"check query {query_ref.label}",
                            timeout=5,
                            delay=0.2,
                            retry_errors=_is_query_reference_error,
                        )
                        if refreshing:
                            active.append(query_ref.label)
                    except Exception as error:
                        if _is_query_status_poll_error(error):
                            active.append(query_ref.label)
                        else:
                            raise
                ...
                now = time.time()
                elapsed = now - refresh_started
                if elapsed > timeout:
                    cancel_query_refreshes(wb, queries)
                    raise Exception(f"Excel query refresh timeout after {int(elapsed)}s")
                if now - last_progress_log >= QUERY_REFRESH_LOG_INTERVAL_SECONDS:
                    write(
                        f"[REFRESH] Batch {batch_number}/{total_batches} waiting "
                        f"for {len(active)} query(s) | {int(elapsed)}s elapsed"
                    )
                    last_progress_log = now
                _pump_excel_messages(0.2)

            write(f"[REFRESH] Batch {batch_number}/{total_batches} completed")
    except Exception:
        cancel_query_refreshes(wb, queries)
        raise

    write(f"[REFRESH] All queries completed in {int(time.time() - refresh_started)}s")
...
def _is_query_reference_error(error: Exception) -> bool:
    text = str(error).lower()
    return (
        "0x800a01a8" in text
        or "-2146827864" in text
        or "-2147352573" in text
        or "member not found" in text
        or "item.listobjects" in text
        or "listobjects.item" in text
        or "item.querytable" in text
    )


def _is_query_status_poll_error(error: Exception) -> bool:
    text = str(error).lower()
    return (
        _is_excel_busy_error(error)
        or _is_query_reference_error(error)
        or "excel busy timeout during check query" in text
    )


def excel_call(
    action: Callable[[], Any],
    write: Optional[LogFunc],
    label: str,
    timeout: int = 180,
    delay: float = 0.7,
    retry_errors: Optional[Callable[[Exception], bool]] = None,
) -> Any:
    start = time.time()
    last_log = 0.0
    attempt = 0
    while True:
        ...
        try:
            ...
            if not _is_excel_busy_error(error) and not (retry_errors and retry_errors(error)):
                raise
            now = time.time()
    ...
    try:
        for i in range(3):
            try:
                ...
                if not excel_pid:
                    write("[WARN] Could not identify the bot-owned Excel PID")
                break
            except Exception as e:
        ...
        def export_range_as_image(wb, item: ExportItem, rng: str) -> None:
            ...
            try:
                excel_call(lambda: setattr(target.Font, "Name", "Microsoft YaHei"), write, f"set font {item.sheet}", timeout=60)
            except Exception:
                pass

            _pump_excel_messages(0.5)
        ...
        for wb_item in workbooks:
            ...
            try:
                excel_app.Calculation = XL_CALCULATION_MANUAL
            ...
            for sheet_index in range(1, sheet_count + 1):
                ...
                sheet_names.add(str(sheet_name).strip().lower())

            for item in group:
                if item.sheet.strip().lower() not in sheet_names:
                    write(f"[SKIP] Missing report sheet: {wb_item.display_name} -> {item.sheet}")
                    continue
        ...
        cleanup_done, cleanup_watchdog = start_excel_cleanup_watchdog(excel_pid, write)
        try:
            for key, workbook in list(opened.items()):
                try:
                    if workbook:
                        excel_call(
                            lambda target=workbook: getattr(target, "Close")(False),
                            write,
                            "close workbook cleanup",
                            timeout=120,
                        )
                except Exception as error:
                    write(f"[WARN] Workbook cleanup failed: {error}")
                finally:
                    opened[key] = None
                    workbook = None
            ...
            try:
                ...
                write(f"[WARN] Excel quit failed: {error}")
        finally:
            # Release every application/workbook proxy before checking whether
            # the bot-owned Excel process needs the forced-cleanup fallback.
            excel_app = None
            excel = None
            gc.collect()
            try:
                ...
                cleanup_done.set()
                if cleanup_watchdog:
                    cleanup_watchdog.join(timeout=1)
